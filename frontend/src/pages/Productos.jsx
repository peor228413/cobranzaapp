...
// Extrae columnas “probables” sin conocer aún el schema exacto
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {/* Botón se activará cuando definamos campos de creación */}
        <button
          className="rounded-lg border px-4 py-2 hover:bg-slate-50"
          disabled
        >
          Nuevo producto (pronto)
        </button>
      </div>

      {items.length === 0 ? (
        <div className="rounded-xl border bg-white p-10 text-center">
          <h2 className="text-xl font-semibold">Sin productos</h2>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                ...
                <th className="px-4 py-3 text-left">Cuenta (ctedectes)</th>
              ...
              {items.map((p) => {
                ...
                const cuenta =
                  pick(p.ctedectes, [
                    ...
                    "numeroCuenta",
                    "alias",
                  ]) || (typeof p.ctedectes === "string" ? p.ctedectes : "-");

                return (
                  <tr key={id} className="border-t">
                    <td className="px-4 py-3">{nombre}</td>
                    <td className="px-4 py-3">{monto}</td>
                    <td className="px-4 py-3">{cuenta}</td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        ...
                        {/* Editar/Eliminar los activamos cuando definamos campos */}
                        <button
                          disabled
